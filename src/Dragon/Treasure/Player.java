...
public class Player {
    ...
    public Player(String name, int startPosY, int startPosX, Item[] backpack) {
    ...
    public void addToBackpack(Item itemToAddInBackpack) {
        Item[] bp2 = new Item[this.backpack.length + 1];
        for (int i = 0; i < this.backpack.length; i++) {
        bp2[i] = this.backpack[i];
        }
        bp2[bp2.length - 1] = itemToAddInBackpack;
        this.backpack=bp2;
        //newInventory = this.inventory; // new inventory ska bestå av this.inventory ;)
    }
}